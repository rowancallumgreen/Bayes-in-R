...
Here's the Statistical Rethinking textbook and youtube lectures etc which are widely recommended as a go to for understanding bayesian statistics: https://xcelab.net/rm/
...
This set of course notes also seems useful: https://bookdown.org/marklhc/notes_bookdown/introduction.html

This book has also been recommended: https://yalebooks.yale.edu/book/9780300188226/the-theory-that-would-not-die/
...
# check systematic discrepancies here
pp_check(fit_mock, nsamples = 100)
pp_check(fit_mock, nsamples = 1000, type = "stat_2d")
pp_check(fit_mock, nsamples = 100, type = "scatter_avg")
pp_check(fit_mock_prior, nsamples = 100)
...
pp_check(fit_imp20, nsamples = 100)

#beta regression in brms
read_csv("ExampleData_Virus.csv")->VirusData

ggplot(VirusData,aes(DVGness,VirusGenotype))+
  geom_boxplot()+
  facet_wrap(~HostGroup,ncol=1)
...
summary(fit_virus)
plot(fit_virus)
#Do the hostgroups differ in DVGness under D1.1 infection
hypothesis(fit_virus, "Intercept + VirusGenotypeD1.1 + VirusGenotypeD1.1:HostGroupWaterfowlAnseriformes + HostGroupWaterfowlAnseriformes = Intercept + VirusGenotypeD1.1")

stanplot(fit_virus, pars = "VirusGenotypeD1.1", type = "areas")
pp_check(fit_virus, nsamples = 100)
```
